...
- BoringSSL prebuilts are replaced with BoringSSL built at compile time
...
- Google Vision face detection and driver's license barcode scanning
...
**Important:**
1. You need the [Android NDK, both revision 16b and 14b](https://developer.android.com/ndk/downloads/older_releases), and [Ninja](https://ninja-build.org/) to build the apk.
...
3. Build native FFmpeg and BoringSSL dependencies:
      - Go to the `TMessagesProj/jni` folder and execute the following (define the paths to your NDKs and Ninja):

      ```
      export NDK=[PATH_TO_NDK_R14B]
